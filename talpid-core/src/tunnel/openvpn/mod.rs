        ...
        async fn route_up(
            ...
            #[cfg(target_os = "linux")]
            {
                ...
                let routes = super::extract_routes(&env)
                    .map_err(|err| tonic::Status::failed_precondition(err.display_chain()))?
                    .into_iter()
            ...
            #[cfg(windows)]
            tokio::task::spawn_blocking(|| super::wait_for_ready_device(tunnel_alias))
                .await
                .map_err(|_| tonic::Status::internal("task failed to complete"))??;
            ...
            (self.on_event)(super::TunnelEvent::Up(crate::tunnel::TunnelMetadata {
                interface: tunnel_alias.to_string(),
                ips,
                ipv4_gateway,
                ipv6_gateway,
            }));
